...
def test_edge_conv():
    ...
    lframes = RandomLFrames()(pos=pos)
    feature_transform = in_reps.get_transform_class()

    # Perform the forward pass
    x_local = feature_transform(coeffs=x.clone(), basis_change=lframes)
    output = edge_conv(x=x_local, pos=pos, edge_index=edge_index, lframes=lframes, batch=batch)
    ...
    # Define the input and output representations
    in_reps = Irreps("16x0n + 8x1n + 2x1p + 4x2")
    ...
    # create some dummy data
    edge_index = torch.randint(0, 100, (2, 20), dtype=torch.long)
    x = torch.randn(100, in_reps.dim)
    pos = torch.randn(100, 3)
    batch = torch.zeros(100, dtype=torch.long)
    lframes = LFrames(-rand_matrix(100))
    feature_transform = in_reps.get_transform_class()
    ...
    pos_trafo = torch.stack(pos_trafo)
    x_trafo_local = feature_transform(coeffs=x_trafo.clone(), basis_change=lframes_trafo)
    assert torch.allclose(x_local, x_trafo_local, atol=1e-5)
    output_trafo = edge_conv(
        x=x_trafo_local, pos=pos_trafo, edge_index=edge_index, lframes=lframes_trafo, batch=batch
    )
